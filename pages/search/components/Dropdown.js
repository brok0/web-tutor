...
export default function Dropdown({ optionsList, id, disabled }) {
  ...
  return (
    <div className={disabled ? "opacity-70 relative inline-block text-left pt-2" : " relative inline-block text-left pt-2"} onClick={toggleOptionList}>
      <div>
        <button
          type="button"
      ...
      {isOpened && <OptionList isOpened={isOpened} optionsList={optionsList} onOptionClick={onOptionClick}></OptionList>}
    </div>
  );
}
